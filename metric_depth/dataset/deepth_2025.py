...
#         self.depth_gt_dir = depth_gt_dir
#         self.split = split
#         # print('finetune {}'.format(self.depth_source_dir))
...
#         '''
#             TODO
#             image reshape anything config kitti  (384, 704)
#             depth_gt w/o reshape (1080, 1980)
#         '''
#         im_index = index
#         color_path, depth_path = self.samples[im_index]
#         # print(color_path)
#         image = torch.from_numpy(np.array(Image.open(color_path).convert('RGB')).astype('float32')).permute(2, 0, 1) / 255.
#         image =  F.interpolate(image.unsqueeze(1), size=(392, 714), mode='bilinear').squeeze(1)
#         # import pdb; pdb.set_trace()
#         depth_pil = Image.open(depth_path)
#         depth_np = np.array(depth_pil).astype('float32')
#         if depth_np.ndim == 3:
#             depth_np = depth_np[:, :, 0]  # 取第一个通道
...
#             depth_np = depth_np[:, :, 0]  # 取第一个通道
#         depth_map = torch.from_numpy(depth_np).unsqueeze(0)
#         depth_map = F.interpolate(depth_map.unsqueeze(0), size=(392, 714), mode='bilinear', align_corners=True).squeeze(1)
...
#         # TODO we may need mask too
#         # mask = np.logical_and(depth_map > self.min_depth,
#         #                         depth_map < self.max_depth).squeeze()[None, ...]
        
#         sample = {'image': image, 'depth': depth_map}
#         mask = torch.ones(image.shape, dtype=image.dtype)
#         sample = {'image': image, 'depth': depth_map}
#         sample['valid_mask'] = (torch.isnan(sample['depth']) == 0)
#         sample['depth'][sample['valid_mask'] == 0] = 0
        
#         sample['image_path'] = color_path
...
# if __name__ == '__main__':
#     from torchvision.transforms import Normalize
#     # TODO chk the depth normlize following depth_anything
#     # depth_transform = Normalize([0.0], [1])
#     dataset = SNDataset(depth_transform = Normalize([0.0], [256]))
#     num_len = len(dataset)
#     print("num len: {}".format(num_len))
...
#         print("depth min: {}, max: {}".format(depth_map.min(), depth_map.max()))
#         # guide, gt = data['guide'], data['y'], data['source']
#         # print("Original shapes:", guide.shape, gt.shape, source.shape)
...
#         # from pathlib import Path
#         # # output_dir = 'visualization_zoedepth'
#         # guide_pil.save(f'visualization_anything/sample_{i}_guide.png')
#         # gt_pil.save(f'visualization_anything/sample_{i}_gt.png')
#         # source_pil.save(f'visualization_anything/sample_{i}_source.png')